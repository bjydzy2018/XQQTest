...
public class DefCATA implements Def {
    ...
    public ICataInitListener iCataInitListener = new ICataInitListener() {
        ...
        public void onCataInited(boolean state, int err) {
            if (state) {
                ...
                Log.i("onCataInited", "初始化成功");
            } else {
                msgInitStatus_fail = true;
                Log.e("onCataInited", "初始化失败，err=" + err);
            }
            msgInitStatus = true;
        }
    };
    ...
    public ICataListener iCataListener = new ICataListener() {
        ...
        public void onMsgProc(int iMsg, int wParam, String lParam) {
            ...
            msgResult = true;
            Log.i("onMsgProc", "iMsg:" + iMsg + ",wParam:" + wParam + ",lParam:" + lParam);
        }
    };
                        ...
                        if (p[p.length - 1].equals("1") && p[p.length - 2].equals("1")) {
                            ...
                            for (int i = 1; i < p.length - 2; i++) {
                                try {
                                    ...
                                    Log.e("createSegValuesList", "segvalue 解析异常");
                                    Log.e("createSegValuesList", Arrays.toString(p));
                                    System.exit(0);
                                }
                            }
                            mMode = 4;
