...
def handle_ret_iss():
    ...
    if len(temp_lines) == 2:
        ...
        temp = 3
        while True:
            temp_ret = ret_iss[iss_pcms[iss_pcm_num-temp]]
            if temp_ret == None:
                ret_iss[iss_pcms[iss_pcm_num-temp]] = mvw_score1
                break
            else:
                t = ret_iss[iss_pcms[iss_pcm_num-temp]]
                ret_iss[iss_pcms[iss_pcm_num-temp]] = mvw_score1
                mvw_score1 = t
                temp += 1
...
with open(sys.argv[1]) as fp:
    for line in fp:
        ...
        if line.startswith("E:/project"):
            iss_pcms.append(re.match(".*(SceneId-./.*)", line).group(1))
            iss_pcm_num += 1
            handle_ret_iss()
    ...
    for key in ret_iss:
        fp_ret_iss.write(key+' '+(str(ret_iss[key]) if ret_iss[key]!=None else 'None')+'\n')
    print len(ret_iss)
...
if len(sys.argv) > 1:
    ...
    for i in range(2, len(sys.argv)):
        with open(sys.argv[i]) as fp:
            ...
            # pprint.pprint(ret)
    for key in ret:
        fp_ret.write(key+' '+(str(ret[key]) if ret[key]!=None else 'None')+'\n')
    print len(ret)
    ...
    for key in ret_iss:
        ...
        if ret.has_key(key):
            ...
            if score != score_iss:
                ...
                fp.write(key.decode('utf-8').encode('gbk')+','+(str(score_iss) if score_iss!=None else 'None')+','+(score if score!=None else 'None')+'\n')
            else:
                ret_num += 1
        else:
            print key.decode('utf-8').encode('gbk').ljust(50), (str(score_iss) if score_iss!=None else 'None').ljust(4), 'None'.decode('utf-8').encode('gbk')
            fp.write(key.decode('utf-8').encode('gbk')+','+(str(score_iss) if score_iss!=None else 'None')+','+'None'.decode('utf-8').encode('gbk')+'\n')
