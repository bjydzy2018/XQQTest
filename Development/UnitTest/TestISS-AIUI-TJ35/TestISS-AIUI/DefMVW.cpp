...
double DefMVW::vw_current_time = 0;
...
void DefMVW::onWakeup(
  ...
  const char*	 lParam
	...
	//DEBUG_TRACE_INFOR("lParam ret: %s",lParam);
	++msgWakeup;
	DEBUG_TRACE( "Wakeup Success!");
	vw_current_time = CurrentTime();
}
...
DefMVW::DefMVW() {
	...
	mvwKeyWordPcm = testRes_path + "pcm_src/mvw/SetKeyWord_Global.pcm";
	mvwKeyWordTxt = readFile(testRes_path + "pcm_src/mvw/SetKeyWord_Global.txt", false);
}
