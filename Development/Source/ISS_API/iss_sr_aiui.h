 ...
 */
	...
	ISS_SR_MSG_LOCAL_RECG_END = 20024,

	/** \brief   Whether the cloud data is successfully uploaded; \n
 *          After a user calls the ISSSRUpLoadData to asynchronously upload the data,
 *          the ISS_SR_MSG_UpLoadDataToCloudStatus message will be sent to notify whether the cloud data is successfully uploaded.
 *  \param  wParam     ISSErrID type:\n
						ISS_SUCCESS: The cloud personalized data is successfully uploaded.\n
						ISS_ERROR_FAIL: The cloud upload of the data fails.\n
 *  \param  lParam    const char* type, error message
 */
	ISS_SR_MSG_UpLoadDataToCloudStatus = 20050,
/** \brief For the mixed recognition mode, a message means having cloud result.
 *  \param  wParam    0
 *  \param  lParam    const char* type, cloud result
 */
	ISS_SR_MSG_CloudResult = 20051,
/** \brief For the mixed recognition mode, a message means having local result.
 *  \param  wParam    0
 *  \param  lParam    const char* type, local result
 */
	ISS_SR_MSG_LocalResult = 20052,
/** \brief   Query whether the cloud data is successfully uploaded; \n
*          After a user calls the ISSSRQuerySyncStatus to asynchronously query sync status result,
*          the ISS_SR_MSG_QuerySyncStatusResult message will be sent to notify whether the cloud data is successfully uploaded.
*  \param  wParam     ISSErrID type:\n
ISS_SUCCESS: The cloud personalized data is successfully uploaded.\n
ISS_ERROR_FAIL: The cloud upload of the data fails.\n
*  \param  lParam    const char* type, error message
*/
	ISS_SR_MSG_QuerySyncStatusResult = 20053,
/** \brief This message will be sent to notify AIUIClient state.
*  \param  wParam    0: IDLE
*  \                 1: READY
*  \                 2: WORKING
*  \param  lParam    const char* type, extra info
*/
	ISS_SR_MSG_AIUIClientSTATE = 20054,
	ISS_SR_MSG_SRResult = 20055,

	ISS_SR_MSG_TPPResult = 20056,

    ISS_SR_MSG_TransResult = 20057,
};
...
#define ISS_SR_ENABLE_CONTINUOUS_MODEL  "EnableContinuousModel"
...
#define ISS_SR_PARAM_THEME				"theme"
#define ISS_SR_PARAM_ATTACHPARAMS		"attachparams"
#define ISS_SR_PARAM_NETWORK_STATUS	    "network_status"			///< network status, on or off, for mix sr
#define ISS_SR_PARAM_SCENE	            "scene"
#define ISS_SR_PARAM_PGS			    "pgs"
...
#define ISS_SR_PARAM_IAT_LOGIN_EXT_PARAMS	"IatLoginExtendParams"
#define ISS_SR_PARAM_IAT_RELOGIN			"CloudReLogin"

#define ISS_SR_UPLOAD_TO_LOCAL_AND_CLOUD		(0)
#define ISS_SR_UPLOAD_TO_CLOUD					(1)
#define ISS_SR_UPLOAD_TO_LOCAL					(2)

#define ISS_SR_PARAM_SEOPT_MODE					"seopt_mode"
...
ISSErrID ISSAPI ISSSRUpLoadData(
    HISSSR hISSSRObj, 			/* [In] Recognition handle */
    const char *szData, 		/* [In] Grammar information in the JSON format */
    int iUpLoadMode				/* [In] Whether upload the dictionary only to the cloud. */
);

ISSErrID ISSAPI ISSSRQuerySyncStatus(
	HISSSR hISSSRObj, 			/* [In] Recognition handle */
	const char *szSid			/* [In] Sid */
);
...
/** \brief  Get all version information \n 
 *  \param  hISSSRObj	[In] Recognition handle\n

 *  \retval 	the kernel version infomation
 */
ISSAPI_PORT const char* ISSCALL ISSSRGetAllVersionInfo(HISSSR hISSSRObj);

/** \brief  Reset session \n 
 *  \param  hISSSRObj	[In] Recognition handle\n

 * \retval     ISSErrID                 The error code returned \n
 * \retval     ISS_SUCCESS              Reset session successfully. \n
 * \retval     ISS_ERROR_INVALID_HANDLE Invalid speech recognition handle \n
 * \retval     ISS_ERROR_INVALID_PARA   Invalid parameter value\n
 */
ISSErrID ISSAPI ISSSRResetSession(HISSSR hISSSRObj);
 ...
 * \retval     ISS_ERROR_INVALID_PARA   Invalid parameter value\n
 */
ISSErrID ISSAPI ISSSRClearResult(HISSSR hISSSRObj);
