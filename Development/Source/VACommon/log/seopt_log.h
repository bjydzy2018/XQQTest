...
// 日志单体
LOG_DECL_SINGLETON_EX(Log_IO_FILE, Log_Thread_Mutex, Log_Cfg_Heap_Reg, issseopt_log);
 ...
 */
	...
	#define isslog_inst                           issseopt_log::instance()
	...
	#define DEBUG_TRACE_SPEC					  if ( isslog_inst ) isslog_inst->log_spec
	...
	// 能够自动统计性能并打印的
	#define isslog_perf_n(name)              Log_Perf_Helper<Log_Timer, issseopt_log>      __ph__(name);
	#define isslog_perf_s(name,fmt)          Log_Perf_Helper<Log_Timer, issseopt_log>      __ph__(name); __ph__.log(fmt);
	#define isslog_perf_nsec(name,nsec)      Log_Perf_Helper<Log_Timer, issseopt_log>      __ph__(name, nsec);
	#define isslog_perf_stop()                                                       __ph__.stop()

	// 跟踪函数调用的方法
	#define isslog_tracer(sub)               Log_Func_Tracer<issseopt_log>                 __lt__(sub);
	#define isslog_tracer_perf(sub)          isslog_perf_n(sub); isslog_tracer(sub);

	// 初始化和逆初始化
	#define isslog_open                      issseopt_log::open
	#define isslog_close                     issseopt_log::close
	#define isslog_open_ac                   Log_Auto_Close<issseopt_log> __lac__; issseopt_log::open
...
#endif
// 配置类
typedef issseopt_log_Cfg                      isslog_cfg;
