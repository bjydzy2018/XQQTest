 ...
 *		13. 提供了多种样式可供选择，包括时间、主题、等级、线程ID、进程ID、自动回车，可以组合；
 *		14. 不依赖于其他头文件，直接拷贝然后include就可以使用；
 *		15. 基于接口的模块间桥接机制，使得日志的代码更新不需要重新编译牵连的模块，多个模块之间可以
 *			实现安全的日志串联共享；
 *		16. 支持日志自动备份机制，支持日志大小、数量的限制，当日志数量达到一定数量（缺省20）时，
 *			会自动删除最早备份的文件；
 *
 * 用法举例：
 ...
 *			jdyu@iflytek.com，谢谢；
 *		3. Just enjoy it!
 *		4. close()时需要注意(放在最后，无写操作时)，close()和log_msg_i没有完全互斥，会存在问题
 * 
 * @author	jdyu
 * @version	1.0
 * @date	2004-05-13
 ...
 *  <tr> <td>1.0	<td>2016-04-07		<td>jjzhuang<td>QNX 崩溃LOG_MAXMSGLEN->8*1024,vsprintf->vsnprintf</tr>
 * </table>
 */
...
#define ENABLE_LOG_PERF_HELPER_MSG 0
...
//by jjzhuang 2016/04/07 QNX 栈空间太大会崩溃
#ifdef WIN32
    const int LOG_MAXMSGLEN			= 80 * 1024;
	const int LOG_HEADMSGLEN		= 1024;								//每条log开头会包含时间、线程id等信息，预留这些长度存储
	const int LOG_EACHMSGLEN		= LOG_MAXMSGLEN - LOG_HEADMSGLEN;	//msg太长，进行分段存储，每段msg的长度
#else
    const int LOG_MAXMSGLEN			= 8 * 1024;
	const int LOG_HEADMSGLEN		= 1024;
	const int LOG_EACHMSGLEN		= LOG_MAXMSGLEN - LOG_HEADMSGLEN;
#endif
	const int SUBJECT_SIZE			= 32;
namespace VA {
//edit by zlchen3
//在centOS环境下，由于调用层次过深，后续申请栈空间过大，导致程序奔溃，修改申请空间的大小
...
public:
	Log_Cfg_T(const char* file = LOG_DEFFILE, const char* ttl = LOG_DEFTITLE, int md = lgm_text)
		...
		, start_time_	(0)
		, start_tick_	(0)
		, perf_thres_	(1)// ->1 so log_perf can log fine
		{
		file_name(file);
		title(ttl);
...
#else
				LOG::instance()->log_perf("%s %.03f msec.", name_, msec);
#endif
			}
		if ( ext_nsec_ )
			*ext_nsec_ = nanosec_;
...
public:
	...
	char			name_[256];
//edit by zlchen3 
//在centos环境下，由于调用层次过深，在这种情况下，申请了10K的栈空间导致栈奔溃，
//从而导致程序奔溃，因此修改为1K，特此说明

//by jjzhuang 2016/04/08 QNX660 10K的栈空间导致栈奔溃
#if ENABLE_LOG_PERF_HELPER_MSG
#ifdef WIN32
    char			msg_ [10*1024];
#else
...
// 跟踪函数调用的方法
# define IFLY_TRACER(name)				Log_Func_Tracer<IFLY_Logger>				__ilt__(#name);
# define IFLY_TRACER_PERF(sub)			IFLY_LOGPERF_N(sub); IFLY_TRACER(sub);
