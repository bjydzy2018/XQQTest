...
inline MD5::MD5(const char *text, unsigned int nLen)
{  
    init();
    update(text, (size_type)nLen);  
    finalize();
}
...
{  
    ...
    m_u32State[0] = 0x67452301;  
    m_u32State[1] = 0xefcdab89;  
    m_u32State[2] = 0x98badcfe;  
    m_u32State[3] = 0x10325476;  
}  
  
//////////////////////////////  
  
// decodes input (unsigned char) into output (uint4). Assumes len is a multiple of 4.  
inline void MD5::decode(uint32 output[], const uint8 input[], size_type len)  
{  
...
// a multiple of 4.  
inline void MD5::encode(uint8 output[], const uint32 input[], size_type len)  
{  
...
inline void MD5::transform(const uint8 block[blocksize])  
{  
    uint32 a = m_u32State[0], b = m_u32State[1], c = m_u32State[2], d = m_u32State[3], x[16];  
...
{  
    ...
    size_type index = m_u32Count[0] / 8 % blocksize;  
  
    // Update number of bits  
    if ((m_u32Count[0] += (length << 3)) < (length << 3))  
        m_u32Count[1]++;  
    m_u32Count[1] += (length >> 29);  
    ...
    {  
        ...
        memcpy(&m_byBuffer[index], input, firstpart);  
        transform(m_byBuffer);  
    ...
    memcpy(&m_byBuffer[index], &input[i], length-i);  
}  
...
inline MD5& MD5::finalize()
{
    ...
    if (!m_bFinalized) {  
        ...
        encode(bits, m_u32Count, 8);  
  
        // pad out to 56 mod 64.  
        size_type index = m_u32Count[0] / 8 % 64;  
        ...
        encode(m_u8Digest, m_u32State, 16);  
  
        // Zeroize sensitive information.  
        memset(m_byBuffer, 0, sizeof m_byBuffer);  
        memset(m_u32Count, 0, sizeof m_u32Count);  
...
inline int MD5::hexdigest(char* buf) const  //char[33]
{
    if (!m_bFinalized){
        return -1;
    } 
    for (int i=0; i<16; i++){
        sprintf(buf+i*2, "%02x", m_u8Digest[i]);
    }
    buf[32]=0;
    return 0;
}
